        ...
        <div className="hidden md:flex space-x-2 items-center" ref={dropdownRef}>
          {user && (
            <>
              ...
              {user.role === 'admin' && (
                <Link to="/admin" className={linkStyle('/admin')} aria-label="Admin Dashboard">
                  Admin
                </Link>
              )}
              {user.role === 'admin' && (
                <Link to="/admin/books" className={linkStyle('/admin/books')} aria-label="Manage Books">
                  Manage Books
                </Link>
              )}
            </>
            ...
            {user && (
              <>
                ...
                {user.role === 'admin' && (
                  <Link
                    to="/admin"
                    className={`block px-3 py-2 rounded-md ${pathname === '/admin' ? 'bg-white text-blue-600 font-semibold' : 'hover:bg-blue-600'}`}
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Admin
                  </Link>
                ...
                {user.role === 'admin' && (
                  <Link
                    to="/admin/books"
                    className={`block px-3 py-2 rounded-md ${pathname === '/admin/books' ? 'bg-white text-blue-600 font-semibold' : 'hover:bg-blue-600'}`}
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Manage Books
                  </Link>
