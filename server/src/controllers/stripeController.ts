...
export const createCheckoutSession = async (
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      locale: 'en',
      success_url: `${(process.env.CLIENT_URL || 'https://library-task-manager.vercel.app').replace(/\/$/, '')}/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${(process.env.CLIENT_URL || 'https://library-task-manager.vercel.app').replace(/\/$/, '')}/cancel`,
    });
